...
def process_unet_png(file_path):
    """Lit une image PNG issue du UNet."""
...
def launch_comparison_viewer(run_dir):
    ...
    unet_base_dir = Path("..") / "change_detection" / "UNet" / "data" / run_name / "outputs"
    unet_pred_dir = unet_base_dir / "prediction"
    unet_td_dir = unet_base_dir / "temporal_difference"
    
    if not pred_dir.exists():
        print(f"Erreur: Dossier 'predictions' introuvable dans {run_dir}")
        return

    # 1. Regrouper tous les fichiers disponibles (basé sur les prédictions/références)
    scenes = {}
    ...
    for p in pred_dir.rglob("*.tif*"):
        if "_pred" in p.name or "_ref" in p.name:
            ...
            if date_str not in scenes[loc_id]:
                scenes[loc_id][date_str] = {
                    'ref': None, 'pred': None, 
                    'unet_pred': None, 'unet_td': None, 
                    'base_name': base_name
                }
    ...
    # Associer les fichiers UNet (PNG)
    for loc_id, dates in scenes.items():
        for date_str, data in dates.items():
            base_name = data['base_name']
            
            expected_png_name = f"{base_name}.png"
            
            # UNet Prediction Mask
            unet_pred_file = unet_pred_dir / expected_png_name
            if unet_pred_file.exists():
                data['unet_pred'] = unet_pred_file
                
            # UNet Temporal Difference
            unet_td_file = unet_td_dir / expected_png_name
            if unet_td_file.exists():
                data['unet_td'] = unet_td_file
    ...
    # MODIFICATION 1 : Le titre du haut est maintenant la Prédiction
    ax_cd.set_title("Détection de Changement (UNet)", fontsize=14)
    ...
    # MODIFICATION 2 : Le titre du bas est maintenant la Temporal Difference
    ax_naive_cd.set_title("Détection de Changement (UNet)", fontsize=14)
    ...
    def load_data(item):
        ...
        unet_pred_img = process_unet_png(item['current']['unet_pred'])
        unet_td_img = process_unet_png(item['current']['unet_td'])
        
        prev_ref = None
        if item['prev'] and item['prev']['ref']:
            prev_ref = process_tiff(item['prev']['ref'])
            
        return cur_ref, cur_pred, unet_td_img, prev_ref, unet_pred_img
    ...
    def update_display():
        ...
        cur_ref, cur_pred, unet_td_img, prev_ref, unet_pred_img = cache[idx].result()
        ...
        disp_td = unet_td_img if unet_td_img is not None else black_img_rgb
        disp_unet_pred = unet_pred_img if unet_pred_img is not None else black_img_rgb
        ...
        # MODIFICATION 3 : Remplacement de 'viridis' par 'gray' et inversion des variables affichées
        cmap_unet = 'gray' if (unet_pred_img is not None and unet_pred_img.ndim == 2) else None
        im_refs['cd'] = ax_cd.imshow(disp_unet_pred, cmap=cmap_unet) # En haut (ax_cd) = Prediction
        
        cmap_td = 'gray' if (unet_td_img is not None and unet_td_img.ndim == 2) else None
        im_refs['naive_cd'] = ax_naive_cd.imshow(disp_td, cmap=cmap_td) # En bas (ax_naive_cd) = Temporal Difference
